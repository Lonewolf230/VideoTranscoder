  ...
  useEffect(() => {
    const poll = async () => {
      const activeJobs = jobsRef.current.filter(
        (j) => !TERMINAL.includes(j.processStatus)
      );
      if (activeJobs.length === 0) return;
      console.log("Polling")
      try {
        const res = await axios.get(
          `${base_url}/all-process-status`
        );
        console.log("Poll response:", res.data)
        const data: { id: number; status: ProcessStatus; created_at: string }[] = res.data;
        data.forEach((d) => {
          console.log(`Updating video_id ${d.id} to status ${d.status}`);
          updateJobStatus(d.id, d.status, d.created_at);
        })
      } catch {
